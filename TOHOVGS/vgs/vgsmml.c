...
struct VgsBgmData *
__stdcall vgsmml_compile_from_file(const char *path, struct VgsMmlErrorInfo *err) {
...
struct VgsBgmData *
__stdcall vgsmml_compile_from_memory(void *data, size_t size, struct VgsMmlErrorInfo *err) {
    struct VgsBgmData *result;
    int nLine, cLine;
    int i;
    int *pos;
...
struct VgsBgmData *
__stdcall vgsmml_compile_from_memory2(const void *data, size_t size, struct VgsMmlErrorInfo *err) {
    struct VgsBgmData *result;
    char *buf;
...
/* phase-4: compile */
static struct VgsBgmData *
phase4(struct MML *mml, char *buf, int *pos, struct VgsMmlErrorInfo *err) {
    char *cp;
    struct _NOTE note;
    struct Channel *ch;
    int i;
    ...
    for (i = 0; pos[i] != -1; i++) {
        ...
        switch (buf[pos[i]]) {
            ...
            case 'c': /* channel */
            case 'C':
                if (('h' != buf[pos[i] + 1] && 'H' != buf[pos[i] + 1]) ||
                    !isdigit(buf[pos[i] + 2])) {
